...
void nfc_stock_ui_configure(
    ...
    view_dispatcher_add_view(view_dispatcher, NfcStockViewSubmenu, submenu_get_view(ui->submenu));
    view_dispatcher_add_view(view_dispatcher, NfcStockViewWidget, widget_get_view(ui->widget));
    view_dispatcher_add_view(
        view_dispatcher, NfcStockViewTextInput, text_input_get_view(ui->text_input));
    view_dispatcher_add_view(
        view_dispatcher, NfcStockViewInventory, variable_item_list_get_view(ui->inventory_list));
    view_dispatcher_add_view(
        view_dispatcher, NfcStockViewEdit, variable_item_list_get_view(ui->edit_list));
}
...
void nfc_stock_ui_apply_main_menu(
    ...
    submenu_add_item(
        ui->submenu, "Scan tag", NfcStockSceneScan, nfc_stock_ui_submenu_callback, scene_manager);
    submenu_add_item(
        ui->submenu,
        "Inventory",
        NfcStockSceneInventory,
        nfc_stock_ui_submenu_callback,
        scene_manager);
    submenu_add_item(
        ui->submenu,
        "Settings",
...
void nfc_stock_ui_show_item(NfcStockUi* ui, const StockItem* item) {
    ...
    const char* location = (item->location[0] != '\0') ? item->location : "(empty)";

    size_t pos = 0;
    for(uint8_t i = 0; i < item->uid_len && pos + 2 < sizeof(uid_hex); i++) {
        int written = snprintf(uid_hex + pos, sizeof(uid_hex) - pos, "%02X", item->uid[i]);
        if(written <= 0) {
            break;
        }
        pos += (size_t)written;
    ...
    widget_add_string_element(ui->widget, 0, 0, AlignLeft, AlignTop, FontPrimary, buffer);

    snprintf(
        buffer,
        sizeof(buffer),
        "Stock: %" PRId32 "  Min: %" PRId32,
        item->quantity,
        item->min_quantity);
    widget_add_string_element(ui->widget, 0, 14, AlignLeft, AlignTop, FontPrimary, buffer);

    snprintf(buffer, sizeof(buffer), "Location: %s", location);
    widget_add_string_element(ui->widget, 0, 28, AlignLeft, AlignTop, FontPrimary, buffer);

    snprintf(buffer, sizeof(buffer), "UID: %s", uid_hex);
    widget_add_string_element(ui->widget, 0, 42, AlignLeft, AlignTop, FontPrimary, buffer);

    widget_add_string_element(
        ui->widget, 0, 55, AlignLeft, AlignTop, FontSecondary, "Up/Down +/-  OK:Edit  Back:Save");
}
