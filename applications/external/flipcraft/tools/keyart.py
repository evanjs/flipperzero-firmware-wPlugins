...
# Blocks
AIR, GRASS, DIRT, STONE, COBBLE, LOG, LEAVES, PLANK = (
    0x0,
    0x1,
    0x2,
    0x3,
...
MESH = {
    GRASS: {
        "tex": [(TEX_EMPTY, 0b1010), (TEX_DIRT, 0b1000), (TEX_GRASSSIDE, 0b1000)],
        "quads": [],
    },
    ...
    COBBLE: _cube(
        TEX_COBBLE, 0b1000, TEX_COBBLE, 0b1000, TEX_COBBLE, 0b1000, TEX_COBBLE, 0b1000
    ),
    LOG: _cube(
        TEX_LOGTOP, 0b1000, TEX_LOGTOP, 0b1000, TEX_LOGSIDE, 0b1000, TEX_LOGSIDE, 0b1000
    ),
    LEAVES: _cube(
        TEX_LEAVES, 0b1000, TEX_LEAVES, 0b1000, TEX_LEAVES, 0b1000, TEX_LEAVES, 0b1000
    ),
    PLANK: _cube(
        TEX_PLANK, 0b1000, TEX_PLANK, 0b1000, TEX_PLANK, 0b1000, TEX_PLANK, 0b1000
    ),
    ...
    SAPLING: {
        "tex": [(TEX_SAPLINGLIGHT, 0b0100), (TEX_SAPLINGDARK, 0b0110)],
        "quads": [
            (QUAD_CROSS1, 0),
            (QUAD_CROSS1, 1),
            (QUAD_CROSS2, 0),
...
class Renderer:
    ...
    def set_cam_rot(self, pitch_deg, yaw_deg):
        ...
        self.m = [[cc, 0.0, sc], [sb * sc, cb, -sb * cc], [-cb * sc, sb, cb * cc]]
    ...
    def raster_tri(self, A, B, C):
        ...
        for y in range(min_y, max_y + 1):
            ...
            for x in range(min_x, max_x + 1):
                outside = (
                    (e0 < 0 or e1 < 0 or e2 < 0)
                    if pos_area
                    else (e0 > 0 or e1 > 0 or e2 > 0)
                )
    ...
    def backfacing(v1, v2, v3):
        return (
            (v3[0] - v1[0]) * (v1[1] - v2[1]) - (v1[1] - v3[1]) * (v2[0] - v1[0])
        ) < 0.0
    ...
    def draw_block_quad(self, x, y, z, quad_id, tex_id, settings, lit=0, mask=None):
        ...
        corners = [
            (bx + t[i][0], by + t[i][1], bz + t[i][2], QUAD_UVS[i][0], QUAD_UVS[i][1])
            for i in range(4)
        ]
    ...
    def bake_face(self, scene, gx, y, gz, quad):
        ...
        def ray(u, v):
            ...
            return self.sun_visible(
                scene,
                base[0] + du[0] * fu + dv[0] * fv,
                base[1] + du[1] * fu + dv[1] * fv,
                base[2] + du[2] * fu + dv[2] * fv,
                own,
            )
    ...
    def render_scene(self, scene):
        SIDE_QUADS = (
            (QUAD_FULL_NEGX, (-1, 0, 0)),
            (QUAD_FULL_POSX, (1, 0, 0)),
            (QUAD_FULL_NEGZ, (0, 0, -1)),
            (QUAD_FULL_POSZ, (0, 0, 1)),
        )
