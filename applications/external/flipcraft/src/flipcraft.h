...
static_assert(
    RENDER_RADIUS_BLOCKS <= ((WINDOW_CHUNKS - 1) / 2) * CHUNK_SIZE,
...
inline ActiveWindow
    activeWindowAround(int blockX, int blockZ, int worldSX = WORLD_SX, int worldSZ = WORLD_SZ) {
    ActiveWindow w;
    w.x0 = blockX - RENDER_RADIUS_BLOCKS;
    if(w.x0 < 0) w.x0 = 0;
    w.x1 = blockX + RENDER_RADIUS_BLOCKS;
    if(w.x1 > worldSX - 1) w.x1 = worldSX - 1;
    w.z0 = blockZ - RENDER_RADIUS_BLOCKS;
    if(w.z0 < 0) w.z0 = 0;
    w.z1 = blockZ + RENDER_RADIUS_BLOCKS;
    if(w.z1 > worldSZ - 1) w.z1 = worldSZ - 1;
    return w;
}
...
enum Texture : uint8_t {
    ...
    TEX_IRONORE = 0x0B,
    TEX_GLASS = 0x0C,
    TEX_SAPLINGLIGHT = 0x0D,
    TEX_SAPLINGDARK = 0x0E,
    TEX_TABLESIDE = 0x0F,
    TEX_TABLETOP = 0x10,
    TEX_FURNACESIDE = 0x11,
    TEX_FURNACETOP = 0x12,
    TEX_FURNACEFRONTOFF = 0x13,
    TEX_FURNACEFRONTON = 0x14,
    TEX_CHESTSIDE = 0x15,
    TEX_CHESTTOP = 0x16,
    TEX_CHESTFRONT = 0x17,
    TEX_COALITEMDARK = 0x18,
    TEX_STICKITEMLIGHT = 0x19,
    TEX_STICKITEMDARK = 0x1A,
...
enum Quad : uint8_t {
    ...
    QUAD_CROSS1 = 0x06,
    QUAD_CROSS2 = 0x07,
    QUAD_SMALL_NEGX = 0x08,
    QUAD_SMALL_POSX = 0x09,
    QUAD_SMALL_NEGZ = 0x0A,
    QUAD_SMALL_POSZ = 0x0B,
    QUAD_SMALL_NEGY = 0x0C,
    QUAD_SMALL_POSY = 0x0D,
    QUAD_ITEMSHADOW = 0x0E,
    QUAD_BLOCKITEM_NEGX = 0x0F,
    QUAD_BLOCKITEM_POSX = 0x10,
    QUAD_BLOCKITEM_NEGZ = 0x11,
    QUAD_BLOCKITEM_POSZ = 0x12,
    QUAD_BLOCKITEM_NEGY = 0x13,
    QUAD_BLOCKITEM_POSY = 0x14,
    QUAD_CROSSITEM1 = 0x15,
    QUAD_CROSSITEM2 = 0x16,
...
struct World {
    ...
    int worldSX() const {
        return chunksX * CHUNK_SIZE;
    }
    int worldSZ() const {
        return chunksZ * CHUNK_SIZE;
    }
