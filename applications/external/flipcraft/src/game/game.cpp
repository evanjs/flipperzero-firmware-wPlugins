...
uint8_t Game::spawnRng() {
    spawnRngState ^= spawnRngState << 13;
    spawnRngState ^= spawnRngState >> 17;
    spawnRngState ^= spawnRngState << 5;
    return (uint8_t)((spawnRngState * UINT32_C(0x2545F491)) >> 24);
}
int Game::smul446(int a, int b) {
    int r = (s8(a) * s8(b)) >> 6;
    return (int8_t)std::clamp(r, -128, 127);
}
...
bool Game::setup(const GameConfig& config) {
    ...
    if(surfaceY < 0) {
        ...
        playerX = bx * BLOCKSIZE;
        playerZ = bz * BLOCKSIZE;
...
int Game::findBlockEntity(int x, int y, int z) {
    for(size_t i = 0; i < tiles.size(); i++)
        if(tiles[i].active && tiles[i].bx == x && tiles[i].by == y && tiles[i].bz == z)
            return (int)i;
    return -1;
}
...
int Game::allocStorage() {
    for(int i = 0; i < STORAGE_CAPACITY; i++)
        if(!(storageUsed[i >> 3] & (1 << (i & 7)))) {
            storageUsed[i >> 3] |= (uint8_t)(1 << (i & 7));
            return i;
        }
    return -1;
}
void Game::freeStorageSlot(int index) {
    ...
    memset(b, 0, sizeof(b)); // clear in-use on disk
...
Game::RayHit Game::rayCast() {
    ...
    if(world.mobsOn())
        for(int i = 0; i < MAX_MOBS; i++) {
            ...
            int mbx = (m.x + 7) >> 4,
                mbz = (m.z + 7) >> 4; // dormant off-ring bodies are not hittable
            ...
            if(cz < (float)CLIP || cz - (float)(MOBWIDTH / 2) >= bestW)
                continue; // body front, not centre
            ...
            float ex = fabsf(cx) * s -
                       (float)(MOBWIDTH / 2) * s; // gap from the crosshair to the body box
...
void Game::handleBreakAndPlace(const Input& in) {
    ...
    if(in.breakPressed && id != -1) {
            ...
            if(drop == 0xFF) { // leaves
                if(strength == STRENGTH_IRON)
                    ...
                    if(r < LEAVES_SAPLING_PROBABILITY)
                        createEntity(bx, by, bz, ENTITY_SAPLING);
                    else if(r < LEAVES_STICK_PROBABILITY)
                        createEntity(bx, by, bz, ENTITY_STICK);
        ...
        while(true) {
            yy++;
            uint8_t a = world.getBlock(bx, yy, bz);
            if(a == BLOCK_SAND) {
                createEntity(bx, yy, bz, ENTITY_FALLINGSAND);
                world.setBlock(bx, yy, bz, BLOCK_AIR);
            } else if(a == BLOCK_SAPLING) {
                createEntity(bx, yy, bz, ENTITY_SAPLING);
                world.setBlock(bx, yy, bz, BLOCK_AIR);
            } else
                break;
        }
    ...
    if(in.placePressed) {
        ...
        if(world.creative()) {
            ...
            uint8_t item = c.type;
            bool placeable =
                !(item == 0 || c.count == 0 || (item >= ITEM_IRONINGOT && item < ITEM_TABLE) ||
                  item == ITEM_COAL || item == ITEM_GUNPOWDER);
            if(!placeable) return;
            blockId = (item == ITEM_DYNAMITE) ?
                          BLOCK_DYNAMITE :
        ...
        if(blockId == BLOCK_SAPLING) {
            ...
            if(below != BLOCK_DIRT && below != BLOCK_GRASS) return;
        }
        if(blockId != BLOCK_SAPLING) {
            int blockX = px * BLOCKSIZE, blockY = py * BLOCKSIZE, blockZ = pz * BLOCKSIZE;
            bool overlapsPlayer = playerX < blockX + BLOCKSIZE && playerX + PLAYERWIDTH > blockX &&
        ...
        if(blockId == BLOCK_SAND) {
            uint8_t below = world.getBlock(px, py - 1, pz);
            if(below == BLOCK_AIR) {
                createEntity(px, py, pz, ENTITY_FALLINGSAND);
            } else
                world.setBlock(px, py, pz, BLOCK_SAND);
        } else
            world.setBlock(px, py, pz, (uint8_t)blockId);
        if(blockId == BLOCK_CHEST || blockId == BLOCK_FURNACE) {
            BlockEnt b;
            b.active = true;
            b.isChest = (blockId == BLOCK_CHEST);
            b.bx = px;
            b.by = py;
            b.bz = pz;
            b.dir = (pl.rot & 0x0F);
...
void Game::miscInputs(const Input& in) {
    ...
    if(bobAmt < target) {
        bobAmt += BOB_EASE;
        if(bobAmt > target) bobAmt = target;
    } else if(bobAmt > target) {
        bobAmt -= BOB_EASE;
        if(bobAmt < target) bobAmt = target;
    }
}
...
void Game::simulateFurnaces() {
    ...
    for(size_t i = 0; i < tiles.size(); i++) {
        ...
        if(inWin) {
            if(!t.loaded) openTileStorage((int)i);
        } else if(t.loaded && (int)i != loadedTile) {
            flushTileStorage((int)i);
        }
    }
    updateAllFurnaces();
}
    ...
    for(auto& f : tiles) {
        ...
        if(valid && f.fuelTime == 0 && !f.slot[1].empty()) {
            ...
            if(fuel == ITEM_COAL)
                ...
                fuel == ITEM_STICK || fuel == ITEM_SAPLING ||
                (fuel >= ITEM_WOODPICKAXE && fuel <= ITEM_WOODSWORD))
                add = 1;
            else if(fuel == ITEM_PLANK || fuel == ITEM_LOG)
                add = 2;
            if(add) {
                f.fuelTime = add;
...
void Game::doRandomTicks() {
    ...
    for(int i = 0; i < RANDOMTICKSPEED; i++) {
        int x = win.x0 + rng() % wsx, y = rng() % WORLD_SY, z = win.z0 + rng() % wsz;
        uint8_t b = world.getBlock(x, y, z);
        if(b == BLOCK_DIRT) {
            if(!above(x, y, z)) continue;
            bool f = false;
            for(int gx = x - 1; gx <= x + 1 && !f; gx++)
                for(int gy = y - 1; gy <= y + 1 && !f; gy++)
                    for(int gz = z - 1; gz <= z + 1 && !f; gz++)
            ...
            if(!nearLog(x, y, z)) {
                ...
                int r = rng();
                if(r < LEAVES_SAPLING_PROBABILITY)
                    createEntity(x, y, z, ENTITY_SAPLING);
                else if(r < LEAVES_STICK_PROBABILITY)
            ...
            for(int lx = x - 2; lx <= x + 2; lx++)
                for(int ly = lowerTop - 1; ly <= lowerTop; ly++)
            ...
            for(int lx = x - 1; lx <= x + 1; lx++)
                for(int ly = upperTop - 1; ly <= upperTop; ly++)
...
void Game::drawCrosshair() {
    screen.invertRect(
        CROSSHAIR_X - CROSSHAIR_ARM, CROSSHAIR_Y, CROSSHAIR_X + CROSSHAIR_ARM, CROSSHAIR_Y);
...
void Game::drawHotbar() {
    ...
    for(int i = 0; i < 5; i++) {
        ...
        if(cre)
            screen.blockSlot(
                ...
                y,
                10,
                BLOCK_PALETTE[(pl.sel + i - 2 + PALETTE_COUNT * 2) % PALETTE_COUNT],
                false);
        else
    ...
    if(!hudItemTicks && !cre) // tooltip in device.cpp takes their place
        for(int i = 0; i < MAXHEALTH; i++)
            screen.heart(35 + i * 6, 43, i < (int)pl.health);
}
void Game::renderWorld() {
    ...
    for(auto& f : tiles)
        if(f.active) {
            int tex = f.isChest ? TEX_CHESTFRONT :
                                  (f.lit ? TEX_FURNACEFRONTON : TEX_FURNACEFRONTOFF);
...
void Game::worldFrame(const Input& in) {
    if(in.slotScroll) {
        if(world.creative()) {
            int s = (pl.sel + in.slotScroll + PALETTE_COUNT) % PALETTE_COUNT;
            pl.sel = (uint8_t)s;
        } else {
...
Game::SlotList Game::buildSlots(ScreenId s) {
    ...
    for(int r = 0; r < 3; r++)
        for(int c = 0; c < 5; c++)
            add(&pl.inventory[r * 5 + c], 38 + c * 10, 53 - r * 10, 9, false, false);
    // the craft block is the same 31x31 area on both screens, only the grid differs
    if(s == SCR_INVENTORY) {
        int map[4] = {0, 1, 3, 4};
        for(int i = 0; i < 4; i++)
            add(&pl.craftGrid[map[i]], 44 + (i % 2) * 10, 7 + (i / 2) * 10, 9, true, false);
        add(&pl.craftOutput, 76, 12, 9, false, true);
    } else if(s == SCR_CRAFTING) {
        for(int i = 0; i < 9; i++)
            add(&pl.craftGrid[i], 39 + (i % 3) * 10, 2 + (i / 3) * 10, 9, true, false);
    ...
    } else if(s == SCR_CHEST && loadedTile >= 0) {
        BlockEnt& ch = tiles[loadedTile];
        for(int i = 0; i < 10; i++)
            add(&ch.slot[i], 38 + (i % 5) * 10, 8 + (i / 5) * 10, 9, false, false);
    }
    return v;
}
...
void Game::guiFrame(const Input& in) {
    ...
    if((in.navX || in.navY) && !slots.empty()) {
        int csx = slots[cursor].sx, csy = slots[cursor].sy;
        int best = cursor, bestd = 1 << 30;
        for(size_t i = 0; i < slots.size(); i++) {
            int dsx = slots[i].sx - csx, dsy = slots[i].sy - csy;
            if(in.navX > 0 && dsx <= 0) continue;
            if(in.navX < 0 && dsx >= 0) continue;
            if(in.navY > 0 && dsy >= 0) continue;
            if(in.navY < 0 && dsy <= 0) continue;
            int d = dsx * dsx + dsy * dsy;
            if(d < bestd) {
                bestd = d;
                best = (int)i;
            }
