...
static constexpr int8_t kDirX[16] =
...
void Game::trySpawnMob() {
    ...
    int bz = (playerZ + PLAYERHALFWIDTH) / BLOCKSIZE +
             (3 + (int)(((q & 31) * 6) >> 5)) * (1 - (int)((q >> 5) & 2));
    if((unsigned)bx >= (unsigned)world.worldSX() || (unsigned)bz >= (unsigned)world.worldSZ())
        return;

    int by = WORLD_SY - 1;
    while(by > 0 && world.getBlock(bx, by, bz) == BLOCK_AIR)
        by--;
...
void Game::updateAllMobs() {
    ...
    ActiveWindow win =
        activeWindowAround(pxc / BLOCKSIZE, pzc / BLOCKSIZE, world.worldSX(), world.worldSZ());
    ...
    for(int mi = 0; mi < MAX_MOBS; mi++) {
        ...
        if(bx < win.x0 - 4 || bx > win.x1 + 4 || bz < win.z0 - 4 || bz > win.z1 + 4) {
            m.active = false;
            continue;
        }
        ...
        if(m.tamed) {
            ...
            else if(m.mode == MOB_CHASE && m.target == 0xFF && (ldx | ldz) < 24)
                m.mode = MOB_WANDER;
        }
        ...
        if(dmgN && m.mode == MOB_CHASE && !m.cool && !(m.tamed && m.target == 0xFF) && adx < 18 &&
           adz < 18 && m.y < ty + 24 && m.y + hgt > ty) {
            ...
            if(m.target == 0xFF) {
                ...
                if(boomPrey) {
                    bool play = rng() <
                                0x50; // ~31%/bite: lingers by the lit fuse, ~50/50 per duel
                    m.mode = play ? MOB_IDLE : MOB_FLEE;
                    m.timer = play ? 30 : 25;
                    m.cool = 38;
                }
            }
