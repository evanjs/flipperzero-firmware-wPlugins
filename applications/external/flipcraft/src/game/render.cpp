...
static void quadNormal(int q, float n[3]) {
    ...
    const float e0[3] = {
        (float)(t[1][0] - t[0][0]), (float)(t[1][1] - t[0][1]), (float)(t[1][2] - t[0][2])};
    const float e1[3] = {
        (float)(t[2][0] - t[1][0]), (float)(t[2][1] - t[1][1]), (float)(t[2][2] - t[1][2])};
...
__attribute__((noinline)) static bool
    sunVisible(const World& w, float px, float py, float pz, const int own[3], bool& viaTex) {
    ...
    if(vx != own[0] || vy != own[1] || vz != own[2]) {
        ...
        if(id0 != BLOCK_AIR) {
            ...
            viaTex = true;
            int tu = (int)((px - vx) * 8.0f);
            if(tu < 0)
                tu = 0;
            else if(tu > 7)
                tu = 7;
            int tv = (int)((pz - vz) * 8.0f);
            if(tv < 0)
                tv = 0;
            else if(tv > 7)
                tv = 7;
    ...
    for(int i = 0; i < SHADOW_MAX_STEPS; i++) {
        ...
        if(cx != ccx || cz != ccz) {
            ...
            if(base) {
                cmaxY = w.slotMaxY[csx][csz];
                reachY = gReach[csx][csz];
            } else {
                reachY = gRingMaxY;
            }
...
__attribute__((noinline)) static int
    bakeFaceShadow(const World& w, int gx, int y, int gz, int quad, uint8_t* mask) {
...
__attribute__((noinline)) static bool
    carryFace(BakeCarry& c, uint32_t key, int& state, const uint8_t*& mask) {
...
__attribute__((noinline)) static uint32_t faceSun(
    ...
    int litTexels = 0;
    for(int i = 0; i < 8; i++)
        litTexels += __builtin_popcount(src[i]);
...
void Renderer::buildChunkMesh(const World& w, int sx, int sz) {
    ...
    auto emit = [&](int lx, int y, int lz, int quad, uint8_t tex, uint8_t set) {
        if(out) {
            ...
            out[count] =
                key |
                faceSun(w, bake, bx0 + lx, y, bz0 + lz, key, maskScratch, maskCount, carryPtr);
        }
        count++;
    };
            ...
            for(int lz = 0; lz < CHUNK_SIZE; lz++) {
                ...
                for(int lx = 0; lx < CHUNK_SIZE; lx++) {
                    ...
                    if(ft[2].valid) { // side faces
                        ...
                        n = lx < CHUNK_MASK ? row[lx + 1] :
                                              w.getBlock(bx0 + CHUNK_SIZE, y, bz0 + lz);
                        ...
                        n = lz < CHUNK_MASK ? B[y][lz + 1][lx] :
                                              w.getBlock(bx0 + lx, y, bz0 + CHUNK_SIZE);
        ...
        for(int sz = 0; sz < WINDOW_CHUNKS; sz++)
            for(int sx = 0; sx < WINDOW_CHUNKS; sx++) {
                ...
                for(uint32_t f : cm.faces) {
                    ...
                    drawBlockQuad(
                        gx, y, gz, quad, (uint8_t)((f >> 15) & 0xFF), (f >> 23) & 0xF, 0, mask);
                }
            }
}
