...
constexpr uint32_t ACT_PLAY = 0, ACT_INFO = 1, ACT_SETTINGS = 2, ACT_RENAME = 3,
                   ACT_DELETE = 4;
...
enum ListMode { LIST_WORLDS, LIST_TEMPLATES, LIST_ACTIONS };
enum FormMode { FORM_CREATE, FORM_SETTINGS };
enum TextMode { TEXT_NAME, TEXT_SEED, TEXT_TPL_NAME, TEXT_RENAME };
...
enum CreateRow : uint32_t {
    ROW_NAME = 0, ROW_SEED, ROW_MODE, ROW_TYPE, ROW_MOBS, ROW_DIST, ROW_SIZE,
    ROW_CREATE, ROW_EXIT, ROW_CREATE_COUNT
};
enum SettingsRow : uint32_t {
    SROW_MODE = 0, SROW_TYPE, SROW_MOBS, SROW_DIST, SROW_SAVE, SROW_EXIT,
    SROW_COUNT
};
...
void form_enter(void* context, uint32_t index) {
    ...
    if(app->form_mode == FORM_CREATE) {
        ...
        case ROW_CREATE:
            ...
            return;
        case ROW_EXIT: open_worlds(app); return;
        default: return;
        }
    }
    ...
    case SROW_SAVE:
        ...
        open_actions(app);
        return;
    case SROW_EXIT: open_actions(app); return;
    default: return;
    }
}
...
bool nav_callback(void* context) {
    ...
    case VIEW_LIST:
        ...
        break;
    case VIEW_FORM:
        if(app->form_mode == FORM_CREATE) open_worlds(app);
        else open_actions(app);   // settings are discarded unless Save was hit
        break;
    case VIEW_ABOUT:
        ...
        break;
    case VIEW_TEXT:
        if(app->text_mode == TEXT_NAME) return_to_form(app, ROW_NAME);
        else if(app->text_mode == TEXT_SEED) return_to_form(app, ROW_SEED);
        else if(app->text_mode == TEXT_RENAME) open_actions(app);
        else open_templates(app);
        break;
